...
@Slf4j
@Singleton
class sWCGOverlay extends OverlayPanel {
    private final Client client;
    private final sWCGPlugin plugin;
    private final sWCGConfig config;

    String timeFormat;
    private String infoStatus = "Starting...";

    @Inject
    private sWCGOverlay(final Client client, final sWCGPlugin plugin, final sWCGConfig config) {
        super(plugin);
        setPosition(OverlayPosition.BOTTOM_LEFT);
        this.client = client;
        this.plugin = plugin;
        this.config = config;
        getMenuEntries().add(new OverlayMenuEntry(RUNELITE_OVERLAY_CONFIG, OPTION_CONFIGURE, "sWCGuild overlay"));
    }
