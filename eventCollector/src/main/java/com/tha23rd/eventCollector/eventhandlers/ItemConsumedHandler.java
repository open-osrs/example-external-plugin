...
import static net.runelite.api.ObjectID.CANNONBALL;
...
@Singleton
public class ItemConsumedHandler extends EventHandler<ItemConsumed>
{
	//Regex patterns
...
//
	...
	public ItemConsumedHandler(Client client, EventCollectorConfig config) {
		...
		System.out.println("Item consoomer");
	}
	...
	private void onGameTick(GameTick tick)
	{
			...
			(player.getAnimation() == BLOWPIPE_ATTACK))
		{
			...
			{
				// TODO create method to get darts inside blowpipe
				// for now is hard coded to mithril
				buildEntries(MITHRIL_DART);
			}
	...
	private double getAccumulatorPercent()
	{
		...
		if (equipment != null && equipment.getItems().length > EQUIPMENT_CAPE_SLOT)
		{
			...
			// TODO implement method to check if we have vorkaths head on assembler
			// uncomment after
...
//			}
	...
	protected void buildChargesEntries(int itemId, int count) {
		...
		ItemConsumed itemConsumed = new ItemConsumed(calculatedPrice * count, itemId, new Date());
